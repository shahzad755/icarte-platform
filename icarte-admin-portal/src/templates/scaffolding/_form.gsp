	...
	if (hasHibernate) {
       ...
       <div class="form-group md-checkbox">
		...
		<label for="${prefix}${p.name}">
			...
			<% if (required) { %><span class="required" aria-required="true">*</span><% } %>
		</label>
		</div><%} else { %>
	<div class="form-group form-md-line-input \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'has-error')} ${required ? 'required' : ''}">
		${renderEditor(p)}
		<label for="${prefix}${p.name}">
			<g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" />
			<% if (required) { %><span class="required" aria-required="true">*</span><% } %>
		</label>
		<span class="help-block"><g:message code="${domainClass.propertyName}.help.${prefix}${p.name}" default="Enter ${p.naturalName}..." /></span>
	</div>
	<%}%>
<%  } %>
